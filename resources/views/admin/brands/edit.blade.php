@extends('admin.master')

@section('content')
<style>
    .brand-edit-page {
        font-size: 16px !important;
    }
</style>

<div class="brand-edit-page min-h-screen bg-gray-100 py-8">
    <div class="w-full px-4 md:px-6 lg:px-8">
        <div class="mx-auto max-w-4xl">

            <div class="mb-6">
                <h1 class="text-[30px] font-bold leading-tight text-gray-900">Edit Brand</h1>
                <p class="mt-1 text-[15px] text-gray-500">Update brand information</p>
            </div>

            @if(session('success'))
                <div class="mb-5 rounded-[16px] border border-green-200 bg-green-50 px-4 py-4 shadow-sm">
                    <div class="flex items-start gap-3">
                        <div class="pt-0.5 text-green-600">
                            <i class="fa-solid fa-circle-check text-[18px]"></i>
                        </div>
                        <div class="text-[15px] text-green-800">
                            <strong class="font-semibold">Success:</strong>
                            {{ session('success') }}
                        </div>
                    </div>
                </div>
            @endif

            @if ($errors->any())
                <div class="mb-5 rounded-[16px] border border-red-200 bg-red-50 px-4 py-4 shadow-sm">
                    <div class="flex items-start gap-3">
                        <div class="pt-0.5 text-red-600">
                            <i class="fa-solid fa-triangle-exclamation text-[18px]"></i>
                        </div>
                        <div class="text-[15px] text-red-800">
                            <strong class="mb-2 block font-semibold">Please fix the following:</strong>
                            <ul class="list-disc space-y-1 pl-5">
                                @foreach ($errors->all() as $error)
                                    <li>{{ $error }}</li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                </div>
            @endif

            <div class="rounded-[20px] border border-gray-200 bg-white shadow-sm">
                <div class="px-5 py-5 md:px-8 md:py-8">
                    <form method="POST" action="{{ route('admin.brands.update', $brand) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        @include('admin.brands._form', ['brand' => $brand, 'submitText' => 'Update'])
                    </form>
                </div>
            </div>

        </div>
    </div>
</div>
@endsection